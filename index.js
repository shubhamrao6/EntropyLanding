...
function updateLogos(){
    ...
    const logos = document.querySelectorAll('img[src*="bot-icon"]')
    
    logos.forEach(logo => {
        if (isDark) {
            logo.src = logo.src.replace('bot-icon-black.svg', 'bot-icon-white.svg')
        } else {
            logo.src = logo.src.replace('bot-icon-white.svg', 'bot-icon-black.svg')
        }
    })
}
...
function switchProduct(product) {
    ...
    if (product === 'chaosai') {
        iframe.src = 'https://chaosai.dev'
        iframe.title = 'ChaosAI'
        btn.href = 'https://chaosai.dev'
        btn.textContent = 'Try ChaosAI'
    } else if (product === 'knowledgedb') {
        iframe.src = 'https://knowledgedb.dev'
        iframe.title = 'KnowledgeDB'
        btn.href = 'https://knowledgedb.dev'
        btn.textContent = 'Try KnowledgeDB'
    }
}
